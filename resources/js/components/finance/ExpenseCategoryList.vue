...
<template>
    <div class="max-w-3xl mx-auto bg-[#1A1D23] p-6 mt-10 rounded-xl shadow-md">
        ...
        <div class="bg-[#0F1115] p-4 rounded-lg flex justify-between items-center">
            <span class="text-lg font-semibold text-white">{{ cat.name }}</span>
            <div class="space-x-2">
        ...
        <div v-if="cat.children?.length" class="grid grid-cols-1 gap-4 mt-4 sm:grid-cols-2">
            <div
            ...
            >
            <div>
                <span class="font-medium text-white">{{ child.name }}</span>
                <p class="mt-1 text-xs text-gray-400">Pai: {{ cat.name }}</p>
            </div>
            <div class="flex space-x-2">
                <button @click="editCategory(child)" class="px-2 py-1 text-xs bg-yellow-500 rounded hover:bg-yellow-600">
                Editar
                </button>
                <button @click="deleteCategory(child.id)" class="px-2 py-1 text-xs bg-red-500 rounded hover:bg-red-600">
                Excluir
                </button>
            </div>
            </div>
